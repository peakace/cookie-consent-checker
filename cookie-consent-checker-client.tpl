...

{
  ...
  "version": 1,
  "securityGroups": [],
  "displayName": "Cookie Consent Checker Client",
  "brand": {
    "id": "brand_dummy",
    "displayName": "",
  ...
  "description": "To be able to use the Cookie Consent Checker, the Cookie Consent Checker must also be used in the client-side Tag Manager.",
  "containerContexts": [
    "SERVER"
  ]
}
...
[
  {
    ...
    "help": "The Cloud Project must also be stored in the Permissions."
  },
  {
    "type": "TEXT",
    "name": "datasetId",
    ...
    "subParams": [
      {
        ...
        "simpleValueType": true,
        "help": "Enter the same key names as in the client-side tag manager. These names must also match the Big Query Table headers."
      },
      {
        "type": "SIMPLE_TABLE",
        "name": "keyValue",
...
const timestampunix = require('getTimestampMillis');
const BigQuery = require('BigQuery');
const Math = require('Math');

const timestamp = Math.round((timestampunix() / 1000));

if (getRequestPath() === '/ccc') {
    claimRequest();
  ...
  const rows = [{
    ...
    'bannerVersion': params.bannerVersion,
    'source_campaign': params.source_campaign,
  ...
  if(data.ownParamsTrue){
    for (var paramEintrag of data.keyValue) {       
     rows[0][paramEintrag.key] = params[paramEintrag.key];    
    }
  }
 log(rows); 
